...
本ドキュメントは、現行実装のキーマップを `グローバル` と `プリミティブ` に分けて整理した一覧です。  
`プリミティブ` は `編集モード`（`tree` / `diagram`）ごとの差分が分かる形式で記載します。

## グローバル

| キートリガー | ルート | 動作 | 編集モード差分 |
| --- | --- | --- | --- |
...
| `cmd+=` / `cmd+shift+=` / `cmd+shift+;` / `cmd+テンキー+` | `global.zoomIn` | ズームイン | なし |
| `cmd+-` | `global.zoomOut` | ズームアウト | なし |
| `cmd+l` | `global.beginConnectNodeSelection` | node / edge target では Connect Mode を開始し、area target では focused area を画面中央へ寄せ、必要ならズームアウトして全体を表示する | area target での再センタリングは mode 非依存 |
| `ctrl+l` | `global.centerFocusedNode` | Focused Node を中央へ移動 | なし（フォーカス必須） |

## プリミティブ（編集モード差分）

| プリミティブ | キートリガー | Tree (`.tree`) | Diagram (`.diagram`) | 備考 |
| --- | --- | --- | --- | --- |
...
| `delete` | `delete` | 選択 / フォーカスノードを削除 | node 対象: 選択 / フォーカスノードを削除 / edge 対象: 選択 / フォーカス edge を削除 | edge 対象で focused edge を含む複数選択時は一括削除 |
...
| `edit(.toggleFocusedAreaEdgeShapeStyle)` | `cmd+shift+e` | focused area の edge 描画を `legacy/curved/straight` で循環切替 | focused area の edge 描画を `legacy/curved/straight` で循環切替 | node / edge / area target のいずれでも有効。エリア単位で適用 |
...
- モード差分や正規化ルールは上記 `プリミティブ（編集モード差分）` の各行を正本とし、Command Palette も同じ条件系に従う。
- Command Palette では edge 対象時に `Edge: Delete Selected` が表示され、`delete` と同じ削除規則（focused edge を含む複数選択は一括削除）が適用される。
- Command Palette では起動直後（検索語句が空）に `↑` で検索語句履歴を過去方向へ 1 件ずつ呼び出し、`↓` で新しい方向へ戻せる。最新入力まで戻りきった後の `↓` は従来どおり候補リストのスクロールに使える。
- Add Node Mode Selection では `t` / `d` / `enter` に加えて、`↑` / `↓` で選択移動、`esc` でキャンセルが可能。
- Connect Mode では `↑` / `↓` / `←` / `→` で候補移動、`enter` で確定、`esc` でキャンセルが可能。
