...
HOTKEY_VM_DISPLAY_RESOLUTION="${HOTKEY_VM_DISPLAY_RESOLUTION:-1512x982px}"
...
vm_apply_display_resolution_if_configured() {
    if [[ -z "${HOTKEY_VM_DISPLAY_RESOLUTION}" ]]; then
        return 0
    fi

    vm_log "Setting ${HOTKEY_VM_NAME} display -> ${HOTKEY_VM_DISPLAY_RESOLUTION}"
    tart set "${HOTKEY_VM_NAME}" --display "${HOTKEY_VM_DISPLAY_RESOLUTION}" >/dev/null
}
